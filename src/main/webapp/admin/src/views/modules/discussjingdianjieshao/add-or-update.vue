      ...
      <el-row>
        ...
        <el-form-item class="input" v-if="type!='info'"  label="关联表id" prop="refid">
          <el-input v-model="ruleForm.refid" 
              placeholder="关联表id" clearable  :readonly="ro.refid"></el-input>
        </el-form-item>
        <div v-else>
        ...
        <el-form-item class="input" v-if="type!='info'"  label="评论内容" prop="content">
          <el-input v-model="ruleForm.content" 
              placeholder="评论内容" clearable  :readonly="ro.content"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  ...
  methods: {
	    ...
	    this.ro.userid = true;
            continue;
                                ...
                                // 多级联动参数
                                          info(id) {
      this.$http({
        url: `discussjingdianjieshao/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `discussjingdianjieshao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.discussjingdianjieshaoCrossAddOrUpdateFlag = false;
                  this.parent.search();

  ...
  <div class="addEdit-block">
    ...
    >
      <el-row>
        ...
        <div v-else>
	      ...
	      <el-input v-model="ruleForm.jingdiandengji"
                placeholder="景点等级" readonly></el-input>
                                    ...
                                    <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="出游季节" prop="chuyoujijie">
          <el-select v-model="ruleForm.chuyoujijie" placeholder="请选择出游季节">
            <el-option
                v-for="(item,index) in chuyoujijieOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
	      ...
	      <el-input v-model="ruleForm.jiaotongzhuangkuang"
                placeholder="交通状况" readonly></el-input>
                                    ...
                                    <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.jingdianzhaopian" label="景点照片" prop="jingdianzhaopian">
          <file-upload
          tip="点击上传景点照片"
          action="file/upload"
          :limit="3"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.jingdianzhaopian" label="景点照片" prop="jingdianzhaopian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.jingdianzhaopian.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
                                    <el-col :span="12">
          ...
          <el-form-item class="input" label="价格" prop="price">
              <el-input v-model="ruleForm.price" 
                placeholder="价格" readonly></el-input>
                                                                                                                          ...
                                                                                                                          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="景点介绍" prop="jingdianjieshao">
                <el-input
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.jingdianjieshao" label="景点介绍" prop="jingdianjieshao">
                    <span>{{ruleForm.jingdianjieshao}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
                                                            ...
                                                            this.jingdianleixingOptions = "自然类旅游景区,人文类旅游景区,主题公园类景区,社会类旅游景区".split(',')
                                        this.jingdiandengjiOptions = "1A,2A,3A,4A,5A".split(',')
                                        this.chuyoujijieOptions = "春季,夏季,秋季,冬季,全年".split(',')
                                        this.jiaotongzhuangkuangOptions = "畅通,拥堵".split(',')
                                                                                                                                                          },
                                                                                                        // 多级联动参数
                                                                                                                                    info(id) {
      this.$http({
        url: `jingdianjieshao/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `jingdianjieshao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jingdianjieshaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
